...
> 本文件整理了教学过程中用户提出的全部问题与对应的深入回答。按主题分组，方便后面查阅�?> 所有回答均基于 Linux 内核源码（主线版本），并标注了对应内核源码路径�?
---

## 目录

- [软链接：为什么目标不存在也能创建？](#软链接为什么目标不存在也能创建)
- [inode �?lookup() 怎么工作？所�?inode 都在磁盘上吗？](#inode-�?lookup-怎么工作所�?inode-都在磁盘上吗)
- [rm �?inode 和数据什么时候释放？"重启"为什么有效？](#rm-�?inode-和数据什么时候释放重启为什么有�?
- [open() 路径字符为什么要拷贝到内核空间？](#open-路径字符为什么要拷贝到内核空�?
- [fs->pwd 是什么？怎么判断绝对/相对路径？](#fs-pwd-是什么怎么判断绝对相对路径)
- [�?dentry 为什么永远不会被换出？dcache 老化规则？](#�?dentry-为什么永远不会被换出dcache-老化规则)
- [软链接的 inode 是怎么找到的？](#软链接的-inode-是怎么找到�?
...
- [负缓存会不会过期？网络文件系统（NFS）怎么办？](#负缓存会不会过期网络文件系统nfs怎么�?
- [硬链接的误解：为什么在 Git Bash �?inode 号不一样？](#硬链接的误解为什么在-git-bash-�?inode-号不一�?
...
- [dentry 存在时内核是否保�?inode 一定存在？](#dentry-存在时内核是否保�?inode-一定存�?
- [ext2/ext3 �?inode 存储在哪里？](#ext2ext3-�?inode-存储在哪�?
- [dcache 的设计意图是什么？](#dcache-的设计意图是什�?

---

## 软链接：为什么目标不存在也能创建�?
**创建时不做目标检查�?* 内核只做一件事——把目标路径当字符串写入软链接文件的数据块�?
```
ln -s /nonexistent/path link
  �?symlink("/nonexistent/path", "link") 系统调用
    �?VFS 创建一个类型为 S_IFLNK �?inode
    �?文件内容就是字符�?"/nonexistent/path"
    �?返回成功 �?```

直到你访问它（如 `cat link`），VFS 解析路径时发�?inode 类型�?`S_IFLNK`，才调用 `dentry->d_inode->i_op->follow_link()` 读软链接内容，然�?*重新发起 lookup()**。这时候目标不存在才报错�?
...
lookup("/home/user/doc.txt")
  �?  ├─ 1. 检�?dcache（内存中�?dentry 缓存�?  �?    命中 �?直接返回（inode 可能在，也可能被换出了）
...
**关键�?*：lookup() 是一�?VFS 接口（函数指针），每个文件系统自己实现�?所�?inode 都在硬盘有备�?只适用于磁盘文件系统。procfs 等虚拟文件系统的 inode 只在内存中存在，重启就没了�?
---
...
**"重启为什么能释放"**：重启后所有进程终�?�?所�?fd 被关�?�?i_count 归零 �?inode 和对应数据块被回收。技术上没错，但在正常运维中不是推荐手段——更好的做法�?`kill -HUP` 让进程重新打开日志文件�?
时间线推演：

```
初始状态：/tmp/test.txt �?inode #42 [i_nlink=1, i_count=0]
...
不是"放到栈里"这么简单，而是**安全（防 TOCTOU�? 隔离（不同地址空间�? 复用（引用计数）** 三管齐下�?
### �?安全隔离——TOCTOU 漏洞
...
如果内核直接使用用户态的指针，在 `copy_from_user()` �?`link_path_walk()` 之间，用户态可以在另一个线程修改路径字符串内容。这就是 **TOCTOU（Time-of-Check-to-Time-of-Use）安全漏�?*�?
内核的解决方案：
...
### �?地址空间隔离
用户态和内核态使用不同的页表（SMAP/SMEP 保护）。内核不能直接解引用用户态指针——需要通过 `copy_from_user()` / `copy_to_user()`，这些函数会检查地址范围、处理缺页异常�?
### �?引用计数复用
...
if (name[0] == '/') {
    nd->path = current->fs->root;  // 从根目录开�?} else {
...
```
引用来源�?1. super_block->s_root 持有一个引用（只要文件系统挂载着�?2. 每个进程�?fs->root.dentry 持有一个引�?3. mount 结构持有引用
...
| 优先�?| 类型 | 条件 |
|--------|------|------|
| 🥇 最�?| �?dentry | d_inode == NULL |
| 🥈 | 未使用的�?dentry | d_count == 0，且最近未被访�?|
| 🥉 | 最近被访问的正 dentry | 刚用过，移到 LRU 尾部保一�?|
...
```
内存压力到来
  �?kswapd / direct reclaim 激�?    �?shrink_slab() 被调�?      �?super_cache_scan() �?prune_dcache_sb()
...
| NTFS | $FILE_NAME 属性和 $SECURITY_DESCRIPTOR 中的 SID |
| FAT32 | 没有 UID 概念，所有文件显示为 root |
| exFAT | 没有 UID 概念 |

这就是为什么从 FAT32 U 盘拷文件回来 owner 都是 root——因�?FAT 格式根本没有 i_uid 字段。mount 时通过参数指定�?
```bash
...
| 失败原因 | 场景 | 频率 |
|---------|------|------|
| **seqcount 验证失败** | RCU 读的过程中另一�?CPU 修改�?dentry，seq 变了，丢弃结�?| 常见 |
| **d_inode �?NULL** | 正被 d_delete() 摘除过程�?| 较少 |
| **RCU 临界区不�?sleep** | 需�?I/O 重验证的文件系统（如 NFS）直接放�?| 特定场景 |

### 三路径完整逻辑

```
lookup_fast()
   �?   ├─ __d_lookup_rcu() �?命中 �?success ✅（最频繁�?   �?   失败（seq 变了 / 并发�?/ 需�?sleep�?   �?   �?   ├─ __d_lookup() �?命中 �?success ✅（持锁重试�?   �?   失败（真的没�?dentry / inode 已释放）
   �?   �?   └─ lookup_slow() �?磁盘 I/O �?```

**RCU 路径**回答的是"我没看到有人动它，这个结果可�?�?*带锁路径**回答的是"我现在就要一个确定的结果，等也要等到"�?
---

## 负缓存会不会过期？网络文件系统（NFS）怎么办？

### 本地文件系统（ext4/XFS）不会过�?
所有文件操作经过同一个内核，创建文件时自动转正负 dentry�?
```c
// vfs_create() �?创建文件
...
**注意**：RCU 路径下不能做 I/O，所�?NFS �?d_revalidate �?RCU 路径中返�?`-ECHILD`，让 VFS 退到慢速路径再做验证�?
### 不同文件系统的负缓存策略

| 文件系统 | 负缓存有效�?| 原因 |
|---------|------------|------|
| ext4/XFS/btrfs | �?永不过期 | 同一内核管理所有操�?|
| NFS（默认） | �?需要每次验�?| 其他客户端可能在服务器上操作 |
| NFS（acreg 参数�?| ⚠️ 窗口内有�?| `acregmin/acregmax` 控制验证间隔 |
| FUSE | 取决于实�?| 自定�?d_revalidate 行为 |

---

## 硬链接的误解：为什么在 Git Bash �?inode 号不一样？
...
1065432 -rw-r--r-- 2 user user 0 Jun 23 10:00 link.txt  �?一模一样！
```
...
## 硬链接是否创建了另一�?inode�?
**没有�?* 硬链接是创建一个新�?dentry，指�?*已有�?inode**，然后把 inode �?`i_nlink` �?1�?
```c
// fs/namei.c �?vfs_link()
...
dentry 在逻辑上组成树（通过 d_parent、d_subdirs 指针维护子树关系），但查找入口是哈希表—�?*O(1) 时间复杂�?*。dentry 间的"�?用于目录递归删除等遍历场景，不用于查找�?
---
...
�?4 次磁�?I/O。每层都走了完整路径�?
第二次访问同一个路径：
�?所�?dentry 都在 dcache �?�?4 �?lookup_fast→全部命�?�?0 次磁�?I/O�?```

如果中间某层�?dentry �?LRU 回收了（比如 "project" 被回收，�?"home" �?"user" 还在）→ 从被回收的那一层开始重新走磁盘 I/O�?部分缓存"）�?
---

## dentry 存在时内核是否保�?inode 一定存在？
...
### RCU 路径的额外保�?
```c
// 即使 RCU 无锁路径也通过 seqcount 检测并发修改：
...
**�?ext4 完全一样�?* 三种文件系统共用同一套磁盘布局——块组（Block Group）模型�?
```
每个 Block Group�?┌────────┬────────┬────────┬──────────────┬──────────────�?�?Super  �?Group  �?Block  �?Inode        �?Data         �?�?Block  �?Desc   �?Bitmap �?Bitmap       �?Blocks       �?�?(备份) �?       �?       �?             �?             �?└────────┴────────┴────────┴──────────────┴──────────────�?                              �?             �?                       inode 表存这里  文件内容存这�?```

inode 表在块组中的位置�?Group Descriptor 记录。lookup() 在目录文件内容中找到文件�?�?拿到 inode �?�?计算所在块�?�?定位到该块组�?Inode Table �?按偏移量读取 inode 条目�?
**格式化时 inode 总数就定死了**：`inode_count = 分区大小 / inode_ratio`（默�?16384 字节一�?inode）。`tune2fs -l /dev/sda1` 查看具体数值�?
...
```c
// 无数次重复的 I/O
// 每个 cd、ls、cat 都从头来一�?lookup("/home/user/project/doc.txt") {
...
同一文件被打开两次：两�?dentry 指向同一�?inode，inode->i_count = 2。各�?close �?i_count 递减但不�?0。dcache 仍保�?dentry，inode 长期驻留�?
...
| shrink_dcache_parent() | 目录被删�?| 递归清理子目录的所�?dentry |
...
static struct dentry *lookup_slow(struct dentry *dentry, unsigned int flags)
{
    ...
    // 拿到锁后，再查一�?dcache（防止另一�?CPU 已经插入了）
...
注意用的�?`inode_lock_shared()`�?*读信号量**）——同一目录下解�?*不同**文件名时，多�?CPU 可并发进�?lookup_slow，不互斥。只有创�?删除文件（写操作）才拿写锁�?
### 第二道防线：哈希桶自旋锁
...
**不会。因为锁的是 inode，不�?dentry�?* bind mount 的两�?dentry 不同，但它们指向�?*父目�?inode 是同一�?*�?
```
...
锁的持有时间非常短——只覆盖一�?I/O，路径分量解析完马上释放�?*不是一次性锁全部，而是一层层递进、用完即放�?*

---

## 路径越长锁冲突越多？但根目录才是最热的吧？
...
**关键结论：inode 锁的冲突点不�?路径�?，而在"目录�?——即大量进程频繁在该目录下创�?删除文件�?*
...
这是 **"字节偏移 �?页索�?** 的转换�?
`PAGE_SHIFT` 不是每页大小本身，而是**页大小以 2 为底的对�?*�?
```
PAGE_SIZE = 4096 字节�?KB�?PAGE_SHIFT = 12（因�?2^12 = 4096�?```

所�?`offset >> PAGE_SHIFT` 就是把字节偏移量除以 4096，得�?这是第几�?�?
```c
pgoff_t index = iocb->ki_pos >> PAGE_SHIFT;
//              文件偏移量（字节�?>> 12
//              4096      >> 12 = 1  �?�?1 页（�?0 开始计数）
...
**为什么用移位而不是乘除？** 因为位运算是一�?CPU 指令，除法是 ~30 条指令。内核代码中所�?字节 offset �?�?index"的转换都用移位�?
---

## flusher 线程怎么找到脏页？需要扫描所�?page cache 吗？

**不需要扫描全部�?* flusher 线程有更高效的数据结构—�?*每个 inode 的脏页通过红黑树组织在 address_space �?xarray 中，同时整个系统的脏 inode 通过链表串联�?*

### 数据结构�?
```
每个文件系统超级�?  └── s_dirty 链表：所�?包含脏页"�?inode
        �?        ├── inode #42 (i_dirty_list)
        �?    └── xarray 中的脏页位图（哪一页是脏的�?        �?        ├── inode #78 (i_dirty_list)  
        �?    └── xarray 中的脏页位图
...
long wb_writeback(struct bdi_writeback *wb, struct wb_writeback_work *work)
{
    for (;;) {
        ...
        // 检查是否到达阈�?        if (pages_written >= work->nr_pages)
            break;
    }
}
```
...
|-------------------|------------------|
| �?扫描所�?page cache �?| �?只遍�?`s_dirty` 链表 |
| �?检查每个页是否为脏 | �?`s_dirty` 链表中只放有脏页�?inode |
...
把这个与你的上一课问题结合—�?*所有高效内核机制都遵循同样的模式：不是在需要时去扫描全部来找，而是在操作发生时就将自己注册到某个链�?哈希�?树中，让消费者（flusher、回收器）只需遍历已知�?候选列�?�?*

---

## folio 是什么的缩写�?
**不是缩写�?* Folio 是英文单词本身（原意"对折的纸/页码"），在内核中代表"一个或多个连续的物理页组成的容�?。folio 统一�?API——函数签名写 `struct folio *` 时就明确表示"可以操作 1～N 个连续页"�?
新�?API 对应�?
| 老代�?| 新代�?| 含义 |
|--------|--------|------|
| `page->mapping` | `folio->mapping` | 所�?address_space |
...
**关于 filemap_add_folio() 的并发防�?*：它�?lookup_slow 的设计完全一致——`xa_lock` �?`xa_load()` 再查一次，如果另一�?CPU 已插入，就释放自己刚分配的页，复用已有的�?
---

## 预读的判断机制——从第一�?read 就开始？

**会�?* 内核从第一�?read（page cache 缺页）就开始尝试预读�?
```c
...
| 触发条件 | 对应参数 | 默认�?| 效果 |
|---------|---------|--------|------|
| 脏页比例超过阈�?| `dirty_background_ratio` | 10% | 后台 flusher 线程开始写�?|
...
**会恢复�?* 当访问模式从随机恢复为顺序时，预读自动恢复。不需要显式触发——只是窗口大小变了。如果后续请求变成顺序的，窗口从初始值（4 页）重新开始递增膨胀�?
**每次 read 都做判断**——`page_cache_sync_readahead()` 在每�?`filemap_read()` 中都会被调用，但开销极低（比�?`index == ra->start + ra->size` 几个整数），无额�?I/O�?
---
...
因为 **`struct address_space` �?`struct anon_vma` 的地址天然对齐�?8 字节**（由 slab 分配器保证），所以它们地址�?bit 0 永远�?0。内核利用这个天然空闲的位做标志�?
```c
...
**不浪费一丝空�?*——一�?8 字节指针同时承载了有效地址�?8 位）和页面类型标志（�?2 位）�?
---
...
| **数据复制** | 需�?`copy_page_to_iter()` | **不需要复�?*——页表映�?page cache 的物理页 |
| **系统调用开销** | �?| **�?*（缺页异常处理完直接继续�?|
| **文件偏移管理** | 内核管理 `file->f_pos` | 用户管理（指针移动） |
| **page cache 交汇** | file->f_mapping �?address_space | VMA->vm_file->f_mapping �?同一�?address_space |

**read() 多了一次内存复制（page cache �?用户缓冲区），mmap 通过页表映射跳过这次复制。这就是"零拷�?的含义�?* �?mmap 需要先调用 mmap() 建立映射范围，�?read() 不需要这个前置步骤�?
**两者都不需要在读写时重新走 dcache/inode**——open() �?mmap() 调用时已完成了路径解析，后续通过 file 对象 / VMA 中的指针直达 address_space�?
---
...
| 维度 | 答案 |
|------|------|
| flusher 从链表取什么？ | **�?inode**（`list_first_entry(&sb->s_dirty, ...)`�?|
| flusher 对什么调�?writepage�?| **脏页**（遍�?xarray 脏标签） |
| 链表节点是什么？ | **inode**（`inode->i_dirty_list`�?|
| "一个脏�?能触�?inode 入链吗？ | **�?*（`mark_buffer_dirty()` 首次标记脏页时入链） |

为什么以 inode 为单位？因为同一�?inode 的数据块在磁盘上通常连续——一次性回写能最大化磁盘吞吐�?
---

## flusher 遍历 xarray 时会碰到干净页吗�?
**不会�?* xarray 支持**标签遍历（tagged iteration�?*——`find_get_folio_tag(XA_TAG_DIRTY)` 只返回有脏标签的页。底层用的是基数树节点上的标签位图，只查有标签的子树，连"看一眼干净�?的开销都没有�?
> 早期内核�?.6.x 之前）没有标签遍历，flusher 确实会遍�?inode 的所有页逐个检�?`PageDirty()`�?
---
...
| **关心什�?* | "哪些文件有脏页没写回" | "哪些物理页可以被回收" |
| **写脏�?* | �?主要工作 | ⚠️ 为了回收页框才做 |
| **清理页表** | �?不做 | �?`try_to_unmap()` |
| **触发条件** | 时间/比例到期 | 内存压力 |

flusher �?从上往�?——从 inode 出发找脏页来写；kswapd �?从下往�?——从物理页出发，发现是脏文件页就写回+清页表，目的是腾出内存�?
---
...
flusher：只�?dirty
  �?写完后页变干净
  �?页框仍然�?page cache �?  �?页表项仍然有�?  �?进程下次读同一�?�?直接命中 �?
kswapd：要释放物理页框
  �?先确保数据已持久化（�?dirty�?  �?再释放页�?�?页框归还 buddy allocator
  �?必须清页�?�?否则进程访问�?MMU 映射到已被回收的页框
  �?进程下次�?�?缺页 �?从磁盘重新加�?�?```

核心：页表映射的�?*物理页框**，不是磁盘数据。flusher 不碰页框只碰数据，所以页表不需要动；kswapd 要回收页框，必须先清页表�?
---
...
## 磁盘 inode bitmap �?inode table 数量一致吗�?
**完全一致�?* bitmap 的第 N �?�?table 中的�?N �?inode —�?一一对应�?
inode bitmap 是一�?4KB 的块，每�?bit 表示一�?inode 是否被占用。每个块组中 bitmap 只用�?`inodes_per_group` �?bit（如 8192），剩余的是 padding。inode table 则连续存放对应数量的 inode 条目�?56B × 8192 = 512 个块）�?
**为什�?inode 总数�?mkfs 时定死？** 因为 bitmap �?table 在格式化时就已经分配好位置和大小了，之后不能动态扩充�?
---
...
- 二级间接：分层的逻辑块映射，每层都是一个连续块号数�?
所以根据逻辑块号可以快速计算它在哪一级间接、第几个位置——这也是 ext2/ext3 随机访问大文件时慢的原因之一：要读多次间接块才能找到目标物理块号�?
---

## Extent 根节点在 i_block[15] 中的哪个位置�?
**是整�?i_block[15] 数组�?0 字节），不是某个元素�?*

�?inode 设置�?`EXT4_EXTENTS_FL` 标志时，60 字节的语义完全改变——不再被当作 15 �?4 字节块号，而是被整体复用为�?
```
...
- **超过 4 �?extent**：`eh_depth > 0`，根节点变成索引节点，指向子节点所在的�?
大多数文件只需要一�?inode 读入就能获取完整的数据块映射—�?0 字节�?`i_block[]` 空间里已经包含了所�?metadata�?
---

## 目录文件的内容是什么？ext4_lookup() 做什么？
...
就像你打开一个文件夹（你知道它在哪），然后在里面对着列表找你要的文件—�?顺序扫描"只针对当前打开的这一个文件夹的内容�?
---

## ext4 delalloc 批量申请失败会怎么样？
...
- 旧部分和追加部分各自内部连续，互相不需要连�?- extent 树用多个条目描述整体布局
...
**核心思想**：把文件名哈希后，按哈希值范围分桶（bucket），每个桶是一个数据块�?
```
...
满了之后：`journal_start()` 阻塞，等�?checkpoint 回收日志空间。这个阻塞对应用程序可见——write() 会卡住，直到日志空间被回收。默认日志大小通常 128MB，ordered 模式下普通负载很少撑满�?
**日志不是磁盘上的一段独立空间，它是环形缓冲区�?* 日志头指向新事务写入的位置，日志尾指向已 checkpoint 完成的位置。头追上尾就满了�?
---

## ordered 模式的安全缺口—�?写了的数据不一定被引用"
...
这就�?fsync 的作用：它让应用程序知道"成功了还是失败了"。如�?fsync 崩溃了，没返回成�?�?应用程序可以重试。数据耐久性的最终责任在应用程序——必须检�?fsync 的返回值�?
---
...
ordered 模式下，日志里只�?inode、bitmap、extent 树等元数据的"副本"。文件内容直接写到数据区，不进日志。日志只保文件系统结构的一致性，不保文件内容�?
data=journal 模式下数据也写日志，所以每个数据块被写两次（一次日志、一次目标位置），写放大严重，日志也更容易爆满�?
---
...
**代价**：数据被写了两次（日�?+ 目标位置），加上 checkpoint 引起的额外开销。这就是 data=journal 慢的根本原因�?
工程教训：数据库通常不用 data=journal 模式（PG/MySQL 有自己的 WAL）。data=journal 更适合少量小文件写入的桌面/嵌入式场景�?
---

## ordered �?writeback 的核心区�?
**一句话：ordered 要求数据块先到磁盘，元数据日志再提交。writeback 不做这个保证�?*
...
ordered 是默认模式——多花一点性能�?崩溃后文件内容不�?的保障。writeback 只用于明确不关心数据持久性的场景（如临时分区）�?
---
...
2. 格式化函数往缓冲区写内容，如果写满了 �?seq_file 自动加倍（8KB�?6KB�?2KB...�?3. 从头重新格式化，直到缓冲区够用为�?
对于 `/proc/self/status` 等已知大小的文件�?KB 一次就够了。对�?`/proc/self/maps` 等可能很大的文件，seq_file 自动扩容�?
---

## procfs �?inode 什么时候创建？

**不是进程创建时预创建的，而是 lookup() 时按需生成�?*

- **/proc 下的固定文件**（version、cpuinfo）：挂载时注�?proc_dir_entry，但 inode 第一次访问时才通过 proc_lookup() 创建
...
proc_lookup() �?dcache 未命中时�?VFS 调用�?lookup_slow() 触发，第一次访问后 dentry 缓存下来就不再调了�?
---
...
不需�?page cache，不需要磁�?I/O——就是一个内存读取操作。这是所有虚拟文件系统的共性�?
---

## tmpfs 换出�?swap 后，要不要更新磁盘上�?inode�?
**不需要。因�?swap 不是一个文件系统，没有 inode 的概念�?*
